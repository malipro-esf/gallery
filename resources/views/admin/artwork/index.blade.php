                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-12">
                            <div class="card">
                                <div class="card-body">
                                    <a href="{{route('artwork.create')}}" class="btn btn-primary text-white">{{__('titles.create')}}</a>
                                </div>
                            </div>
                        </div>
                    </div>
...
{{--                    <h5 class="card-subtitle">Swipe Mode, ModeSwitch, Minimap, Sortable, SortableSwitch</h5>--}}
                    <table class="tablesaw table-bordered table-hover table" data-tablesaw-mode="swipe" data-tablesaw-sortable data-tablesaw-sortable-switch data-tablesaw-minimap data-tablesaw-mode-switch>
                        ...
                        <tr class="text-center">
                            ...
                            <th scope="col" data-tablesaw-sortable-col data-tablesaw-priority="3">{{__('titles.stock')}}</th>
                        ...
                        <tr class="text-center">
                            <td>{{++$index}}</td>
                            <td>{{$artwork->name_persian}}</td>
                            <td>{{$artwork->name_english}}</td>
                            <td><span class="badge {{$artwork->inventory_status=='sold'?'badge-warning':'badge-success'}}">
                                {{__('titles.'.$artwork->inventory_status)}}</span>
                            </td>
                            <td><img class="img-list" src="{{$artwork->images?asset('images/artworks/'.$artwork->images[0]->url):''}}"></td>

                            <td>
                                <a href="{{route('artwork.show',['artwork' => $artwork])}}"  data-toggle="tooltip"
                                   data-placement="top" title="" data-original-title="{{__('titles.show')}}">
                                    <i class="fa fa-eye"></i>
                                </a>


                                <a href="{{route('artwork.edit',['artwork' => $artwork])}}"  data-toggle="tooltip"
                                   data-placement="top" title="" data-original-title="{{__('titles.edit')}}">
                                    <i class="fa fa-edit"></i>
                                </a>

                                <form action="{{ route('artwork.destroy', ['artwork' => $artwork->id]) }}" method="post" style="display: inline;">
                                    {{ csrf_field() }}
                                    {{ method_field('delete') }}
